 ...
 * (`dist/index.asset.php`) generated by the build tool. If the metadata file
 * is missing, falls back to `filemtime()` for cache-busting the fallback file.
 *
 * @param string $asset_file     Path to the asset metadata file.
...
function design_system_get_asset_data( $asset_file, $fallback_file ) {
    ...
    return array(
        ...
        'version'      => isset( $asset_data['version'] ) ? $asset_data['version'] : filemtime( $fallback_file ),
    );
}
